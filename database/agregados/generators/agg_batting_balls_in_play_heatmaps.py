"""
agg_batting_balls_in_play_heatmaps_sqlite.py
─────────────────────────────────────────────
Equivalent of the MySQL CALL agg_batting_balls_in_play_heatmaps(...) cube inserts,
rewritten as static SQL for SQLite3.

Key differences from the batting_stats scripts:
  - Target table  : agg_batting_balls_in_play_heatmaps
...
Usage:
    python agg_batting_balls_in_play_heatmaps_sqlite.py [--output agg_batting_balls_in_play_heatmaps.sql]
"""
...
def build_insert(grouping: dict) -> str:
    ...
    return f"""
-- ── Grouping {gid}: {gdesc} ──────────────────────────────────────────────
INSERT INTO agg_batting_balls_in_play_heatmaps (
        {cols_str}
)
{build_cte().strip()}
SELECT
...
def generate_sql_file(output_path: str) -> None:
    header = """\
-- ============================================================
-- agg_batting_balls_in_play_heatmaps cube — SQLite3 static equivalent
-- Generated by agg_batting_balls_in_play_heatmaps_sqlite.py
-- Run with:  sqlite3 your_base.db < agg_batting_balls_in_play_heatmaps.sql
-- ============================================================
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate agg_batting_balls_in_play_heatmaps cube SQL for SQLite3"
    )
    parser.add_argument(
